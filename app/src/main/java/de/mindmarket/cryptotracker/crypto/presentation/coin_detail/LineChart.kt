...
fun LineChart(
    ...
    val maxYValue =
        remember(visibleDataPoints) { // calculate the max and min y values to not show additional vertical unused chart areas
            visibleDataPoints.maxOfOrNull { it.y }
        } ?: 0f
    ...
    Canvas(
        ...
        // find the widest and highest text..
        val maxXLabelWidth = xLabelTextLayoutResults.maxOfOrNull { it.size.width } ?: 0
        val maxXLabelHeight = xLabelTextLayoutResults.maxOfOrNull { it.size.height } ?: 0
        val maxXLabelLineCount = xLabelTextLayoutResults.maxOfOrNull { it.lineCount } ?: 0
        val xLabelLineHeight = if (maxXLabelLineCount > 0) maxXLabelHeight / maxXLabelLineCount
        else 0

        // calculate the viewport (the area where the lines are placed)
        val viewPortHeightPx = size.height -
                (maxXLabelHeight + 2 * verticalPaddingPx
                        + xLabelLineHeight + xAxisLabelSpacingPx)

        // calculating the dollar values we want to show on the right side (depending on the crypto)
        ...
        xLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            if (showHelperLines) {
                drawLine(
                    ...
                    strokeWidth = if (selectedDataPointIndex == index) style.helperLinesThicknessPx * 1.8f
        ...
        val heightRequiredForLabels = xLabelLineHeight *
                (labelCountExcludingLastLabel + 1) // now we want all the labels
        ...
        // align the labels at the y axis right aligned (because values can have different length)
        // so calculate the max width of all labels, calculate the x value and add it to the offset
            ...
            if (showHelperLines) {
                drawLine(
                    ...
                    start = Offset(
                        ...
                        x = viewPortRightX,
                        y + result.size.height.toFloat() / 2f
                    ),
        ...
        /*
        Setup the coordinate points related to our Canvas Dimensions
         */
        ...
        /*
        Making the lines between two points curvey!
         */
        ...
        // now we can draw our path with that information..
